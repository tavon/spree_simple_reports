Admin::ReportsController.class_eval do
  ...
  def simple
    ...
    if search[:order_created_at_greater_than].blank?
      search[:order_created_at_greater_than] = Time.now - 3.months
    else
...
#:barWidth => 24*60*60*1000
    ...
    data = @search.all.collect {|i| [i.created_at.to_i * 1000 , i.price * i.quantity ] }
    @flot_options = { :series => {  :lines =>  { :show => true , :fill => true , :steps => true } } ,
                      :xaxis =>  { :mode => "time" }  
                    }
    @flot_data = [{ :label => :all , :data => bucket_array( data , :week )}]
